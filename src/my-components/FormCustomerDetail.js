...
function FormCustomerDetail({ passDataToParent }) {
	let updateCustomerType = (selectedOption) => {
		//selectedOption === "business" (0);
		passDataToParent(updateCustomerType); //Child1
	};
	...
	return (
		<>
			...
			<div class="row">
				<fieldset
					class="border border-dark col-12 col-lg-11 ms-2 me-4"
					onChange={(event) => updateCustomerType(event.target.checked)}
				>
			...
			<div class="row mt-1">
				...
				<input class="col-12 col-md-12 col-lg-7" type="text" pattern="[A-Za-Z][-]" id="fname" required />
			</div>
			<div class="row mt-1">
			...
			<div class="row mt-1">
				...
				<input class="col-12 col-md-12 col-lg-7" type="text" id="postcode" pattern="[0-9]{4}" length="4" />
			</div>
			<div class="row mt-1">
